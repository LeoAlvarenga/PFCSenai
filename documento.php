	...
	<nav class="navbar navbar-expand-lg navbar-dark">
		  ...
		  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
			...
			<div class="btn-group-vertical" role="group" aria-label="Grupo de botões com dropdown aninhado" style="float:left; width: 250px; margin-left: 40px;">
				...
				<div>
					<button type="submit" class="btn btn-success ml-2" title="Pesquisar" value="pesquisar" style="margin-top: 33px;">Pesquisar</button>
				</div>
			...
			if(isset($_POST['tipo'])){
				...
				$result_registros = "SELECT * FROM arquivos WHERE".$sql;
				$resultado_registros = mysqli_query($conn, $result_registros);
			?>
			  
		<div class="container-fluid" style="margin-top: 50px;">
			<table class="table table-hover" style="box-shadow: 0px 0px 10px rgba(0, 0, 0, .6); background-color: #ffffff; opacity:1.0;
	color: rgba(0, 0, 0, 1)">
				  ...
				  <tbody>
					...
					<?php while($rows_registros = mysqli_fetch_assoc($resultado_registros)) { ?>
						...
						<tr >
						  ...
						  <td>
							<input type="hidden" name="editarid" value="<?php echo $rows_registros['id']; ?>" >
						    <a href="editararquivo.php?id=<?php echo $rows_registros['id']; ?>" ><button type="button" title="Editar" class="btn btn-success btn-sm">Editar</button></a>
						 </td>	

						  <td>
						    <a href="funcaoexcluir.php?id=<?php echo $rows_registros['id']; ?>"><button type="button" title="Excluir" onclick="return confirm('Tem certeza que deseja apagar o Documento selecionado?')" class="btn btn-danger btn-sm">Excluir</button></a>
						 </td>
